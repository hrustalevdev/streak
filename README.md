...
**0. Проверить, что модель поддерживает tool calling:**
```bash
curl http://<host>:11434/api/tags | grep -A2 '"name"'
# в capabilities должно быть "tools"
```
...
cp .env.example .env
```
Поменять две строки под свой случай:
```env
# Ollama на этой же машине, что и Docker:
OLLAMA_BASE_URL=http://host.docker.internal:11434/v1
OLLAMA_MODEL=<ваша модель с поддержкой tools>

# Или — Ollama на другой машине в сети:
OLLAMA_BASE_URL=http://192.168.1.X:11434/v1
OLLAMA_MODEL=<модель на этом десктопе>
```

**2. Поднять и дождаться готовности:**
```bash
docker compose up api -d
docker compose ps   # дождаться "healthy"
```

**3. Прогнать тестовые запросы:**
```bash
QUERY="покажи все мои привычки" docker compose run --rm agent
QUERY="создай привычку читать книги" docker compose run --rm agent
QUERY="отметь медитацию выполненной" docker compose run --rm agent
QUERY="какой у меня стрик по медитации?" docker compose run --rm agent
QUERY="что такое streak?" docker compose run --rm agent
```

**4. Как понять, что всё работает:**
- в выводе есть строки `[tool] <имя> → <результат>` — значит агент реально дёргал API, а не выдумывал ответ
- финальный блок строго `Status:` / `Action:` / `Data:` / `Errors:` (английские названия полей, `Status` = `success` или `error`)
...
**Если не работает:** `connection refused` / таймаут на шаге 3 почти всегда означает неверный `OLLAMA_BASE_URL` — перепроверьте `host.docker.internal` (если Ollama на этом же компе) или IP (если на другой машине в сети). `OLLAMA_BASE_URL=http://localhost:...` внутри контейнера работать не будет — `localhost` там означает сам контейнер, а не хост.

Проверено вживую обоими способами (Ollama на хосте через `host.docker.internal`, Ollama на другой машине в сети через LAN-IP) — Docker bridge не изолирует от LAN, доп. настройка сети не требуется.
...
**Известное ограничение:** `qwen3.5:9b` стабильно вызывает правильные tools в правильном порядке (list → create/mark → без дублей и без выдуманных id), но контракт `Status/Action/Data/Errors` в финальном тексте не гарантирован на 100% ни на однозначных, ни на неоднозначных запросах — изредка пропускает поле (обычно `Errors:`) или добавляет текст до/после блока вместо строгого 4-строчного ответа. Наблюдалось в т.ч. на стандартных тестовых запросах из `docs/report.md`, не только на edge-кейсах — см. `docs/progress.md`, раунды 5 и 7. Это вероятностная особенность локальной 9B-модели, программно не валидируется.
